...
@implementation RCTVideo
{
  ...
  BOOL _isPlayerTimeControlStatusObserverRegistered;
  BOOL _videoLoadStarted;

  bool _pendingSeek;
  float _pendingSeekTime;
  float _pendingSeekTolerance;
  float _lastSeekTime;
...
- (void)dealloc
{
  ...
  [self removePlayerIsBufferingObservers];
...
- (void)sendProgressUpdate
{
  ...
  if( currentTimeSecs >= 0 && self.onVideoProgress) {
    self.onVideoProgress(@{
                           ...
                           @"isPlaying": [NSNumber numberWithBool:[self _isPlayerPlaying]],
                           @"isBuffering": [NSNumber numberWithBool:[self _isPlayerBuffering]]
                           });
  }
}
...
- (void) addPlayerIsBufferingObservers
{
  if ([_player respondsToSelector:@selector(timeControlStatus)]) {
    // Only available after iOS 10
    [_player addObserver:self forKeyPath:@"timeControlStatus" options:0 context:nil];
    _isPlayerTimeControlStatusObserverRegistered = YES;
  }
}

- (void) removePlayerIsBufferingObservers
{
  if ([_player respondsToSelector:@selector(timeControlStatus)] && _isPlayerTimeControlStatusObserverRegistered) {
    // Only available after iOS 10
    [_player removeObserver:self forKeyPath:@"timeControlStatus"];
    _isPlayerTimeControlStatusObserverRegistered = NO;
  }
}
...
- (void)addPlayerItemObservers
{
  ...
  [_playerItem addObserver:self forKeyPath:loadedTimeRangesKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)setSrc:(NSDictionary *)source
{
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
    ...
    [self playerItemForSource:source withCallback:^(AVPlayerItem * playerItem) {
      ...
      [self addPlayerIsBufferingObservers];
...
- (void)setSeek:(NSDictionary *)info
{
  ...
  if (item/* && item.status == AVPlayerItemStatusReadyToPlay*/) {
    ...
    _pendingSeekTolerance = [seekTolerance floatValue];
  }
}
